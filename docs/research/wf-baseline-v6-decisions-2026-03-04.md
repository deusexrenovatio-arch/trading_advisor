# Walk-Forward Baseline Decisions (2026-03-04)

## Selected Baseline
- Baseline report: `artifacts/research/wf_goal_v6_mad_neg8_seed111_wt_signal_engine_rerun_20260304.json`
- Why selected:
  - Positive net on wide universe and long causal period.
  - Better TP/SL balance than `robust_normalized` reruns with same constraints.
  - Uses conservative train-coverage and negative-period penalties.

## Baseline Spec
- Universe: 66 instruments (front-nearest roots from prior `v6` runs).
- Period: `2025-03-01` to `2026-03-02`.
- Decision times: `10:30, 12:00, 14:00` (MSK).
- Walk-forward: `train=28`, `test=7`, `step=7`, `retune_every=3`.
- Objective: `robust_median_mad`, `robust_mad_penalty=0.5`.
- Stability penalty: `objective_negative_fold_penalty=8`, `objective_subfold_days=7`.
- Coverage gates: `min_train_trades=80`, `min_train_instruments_with_trades=10`, `min_trades_per_instrument=5`.
- Cost model: `train_proxy_v1`.
- Search profile: `intraday_goal_v3`, algorithm `TPE`, `hpo_trials=24`, `hpo_startup_trials=8`, `seed=111`.

## Rerun Comparison (same baseline window)
- Aggregated table: `artifacts/research/wf_goal_v6_rerun_compare_20260304.json`
- `negpen8 + robust_normalized + prob_gate(10/1.0)`:
  - `net_ticks_sum=5966.0`, folds `14+/11-/28 zero`, `filled=71`.
- `negpen8 + robust_normalized`:
  - same as above (probability gate had no impact in current code/data state).
- `negpen0 + robust_normalized`:
  - `net_ticks_sum=5375.5`, folds `9+/16-/28 zero`, `filled=76`.
- `negpen8 + robust_median_mad` (selected baseline):
  - `net_ticks_sum=6203.0`, folds `14+/11-/28 zero`, `filled=71`.

## Accepted Changes
- Keep negative-period penalty (`objective_negative_fold_penalty=8`): improves fold balance and net vs `negpen0`.
- Keep robust objective family and use `robust_median_mad` baseline for now.
- Keep strict train coverage (`80/10/5`) as baseline contract.

## Rejected / Retired For Now
- `intraday_goal_v4_clustered` search-space profile:
  - no durable uplift in latest comparisons; removed from active runner choices.
- `fold_stability` selection objective:
  - no durable uplift in latest comparisons; removed from active runner choices.
- Probability-gate threshold sweeps (`n_eff=10/20`, `ER=1.0/1.5`) as optimization lever:
  - no measurable effect in current reruns (identical outcomes); keep gate code, stop using this as primary tuning axis.

## Do-Not-Retest List (until data/model regime changes)
- `objective_negative_fold_penalty=0` as primary setting.
- `probability_gate` threshold-only retunes without other structural changes.
- `intraday_goal_v4_clustered` and `fold_stability` in main tuning loop.

## Hypothesis Loop Log (post-freeze)
- H1 (`precision filter`: `min_risk_ticks>=114` + `dedup setup_id`) evaluated on baseline planned signals:
  - artifact: `artifacts/research/wf_goal_v6_hyp1_precision_filter_eval_20260304.json`
  - fast estimate result:
    - raw: `71 filled`, `win_rate=47.89%`, `net=6203.0`
    - dedup only: `55 filled`, `win_rate=43.64%`, `net=4482.0`
    - dedup + H1: `9 filled`, `win_rate=77.78%`, `net=4851.5`
- Full causal reruns on frozen baseline spec:
  - baseline: `artifacts/research/wf_goal_v6_mad_neg8_seed111_wt_signal_engine_rerun_20260304.json`
    - `filled=71`, `win_rate=47.89%`, `net=6203.0`
  - H1 rerun: `artifacts/research/wf_goal_v6_hyp1_precision_filter_rerun_20260304.json`
    - `filled=13`, `win_rate=61.54%`, `net=3075.5`
  - H2 rerun (`+ allow_decision_time=12:00`): `artifacts/research/wf_goal_v6_hyp2_precision_filter_rerun_20260304.json`
    - `filled=3`, `win_rate=66.67%`, `net=2170.5`
  - H3 rerun (`min_risk_ticks=114 + min_target_return_pct=1.5 + dedup`): `artifacts/research/wf_goal_v6_hyp3_precision_filter_rerun_20260304.json`
    - `filled=8`, `win_rate=87.50%`, `net=3658.5`
  - H4 rerun (`min_target_return_pct=1.5 + dedup`, no risk floor): `artifacts/research/wf_goal_v6_hyp4_precision_filter_rerun_20260304.json`
    - `filled=26`, `win_rate=57.69%`, `net=3692.5`
  - H5 rerun (`min_risk_ticks=80 + min_target_return_pct=1.5 + dedup`): `artifacts/research/wf_goal_v6_hyp5_precision_filter_rerun_20260304.json`
    - `filled=9`, `win_rate=77.78%`, `net=3548.0`
- Aggregated comparison and recommendation:
  - artifact: `artifacts/research/wf_goal_v6_precision_hypothesis_compare_20260304.json`
  - hypotheses meeting target (`win_rate>=75%`): `H3`, `H5`
  - recommended: `H3` (highest net among target-meeting candidates).
- Interpretation:
  - Target winrate is reachable in full causal mode, but only with low trade frequency (`8-9` trades over period).
  - Concentration risk remains elevated (`PD` still >54% of net in H3/H5), so next cycle must optimize coverage/diversification without dropping below 75%.

## Methodology Update (Chat-Pro + Internal Review)
- Consolidated methodology and hypothesis-check protocol are fixed in:
  - `docs/research/wf-winrate75-methodology-plan-2026-03-04.md`
  - `artifacts/research/wf_goal_v6_methodology_probe_20260304.json`
- Key addition:
  - treat objective as constrained (`winrate`, `trades/week`, `net`, concentration) instead of single-axis filtering.
  - move next cycle from simple precision filters to `recall -> precision -> exit/risk routing`.
