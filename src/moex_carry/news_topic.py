...
def classify_shock_impact_tier(
    *,
    ...
    episode_signed_increment_pct: float | None = None,
    episode_cum_signed_move_pct: float | None = None,
    episode_event_index: int | None = None,
) -> str:
    z = float(abs(z_score_abs or 0.0))
    move = float(abs(abs_move_pct or 0.0))
    if z >= 4.0 or move >= 2.5:
        base_rank = _IMPACT_TIER_RANK["major"]
    elif z >= 3.0 or move >= 1.5:
        base_rank = _IMPACT_TIER_RANK["strong"]
    elif z >= 2.5 or move >= 1.0:
        base_rank = _IMPACT_TIER_RANK["medium"]
    else:
        base_rank = _IMPACT_TIER_RANK["minor"]

    increment = float(abs(episode_signed_increment_pct or 0.0))
    cum_signed = float(abs(episode_cum_signed_move_pct or 0.0))
    bonus = 0
    if int(episode_event_index or 0) > 1:
        if increment >= 1.0:
            bonus += 1
        elif increment >= 0.6 and z >= 2.5:
            bonus += 1
        if cum_signed >= 3.0 and z >= 2.0:
            bonus += 1

    adjusted_rank = min(base_rank + bonus, _IMPACT_TIER_RANK["major"])
    return _IMPACT_TIER_ORDER[adjusted_rank]
