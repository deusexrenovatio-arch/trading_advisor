...
def derive_news_fingerprint(row: dict[str, Any]) -> str:
    if _normalize_text(row.get("story_key")):
        return f"news:{_normalize_text(row.get('story_key')).lower()}"
...
def format_news_message(alert: dict[str, Any]) -> str:
    ...
    commodity_scope = _normalize_text(alert.get("commodity_scope")).upper()
    direction = _normalize_text(alert.get("direction")).upper() or "N/A"
    ...
    headline = _normalize_text(alert.get("title") or alert.get("headline"))
    url = _normalize_text(alert.get("url"))

    lines = [
        "NEWS IMPACT ALERT",
        f"Commodity: {commodity} | Direction: {direction}",
        f"Impact: {impact:.3f} | Confidence: {confidence:.3f} | Severity: {severity}",
        f"Published: {published_label}",
        f"Source: {source_label}",
    ]
    if commodity_scope:
        lines.append(f"Commodity Scope: {commodity_scope}")
    if headline:
