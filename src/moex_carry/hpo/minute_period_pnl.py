...
def _aggregate_pair_metrics(rows: list[PairWindowMetrics]) -> dict[str, float]:
    ...
    metrics: dict[str, float] = {
        "CAGR": _or_zero(cagr_mean),
        "ExcessAnn": _or_zero(excess_mean),
        "MaxDD": 0.0,
        "AvgTurnover": 0.0,
        "Vol_ann": 0.0,
        "IR": 0.0,
        "r_d": 0.0,
        "b_d": 0.0,
        "ex_d": _or_zero(excess_mean) / 365.0,
        "MinuteRealizedAnnualMean": _or_zero(cagr_mean),
        "MinuteRealizedAnnualMedian": _or_zero(cagr_median),
        "MinuteExcessAnnMean": _or_zero(excess_mean),
        "MinuteExcessAnnMedian": _or_zero(excess_median),
        "MinuteRealizedPeriodReturnMean": _or_zero(realized_period_mean),
        "MinuteUnfilledEntryRateMean": _or_zero(unfilled_entry_mean),
        "MinuteForcedExitRateMean": _or_zero(forced_exit_mean),
        "MinuteIdleRatioMean": _or_zero(idle_ratio_mean),
        "MinuteActiveRatioMean": _or_zero(active_ratio_mean),
        "MinuteAnnualTargetPassRateMean": _or_zero(target_pass_mean),
        ...
        "realized_annualized_return": _or_zero(cagr_mean),
        "excess_annual_vs_target": _or_zero(excess_mean),
        "realized_period_return": _or_zero(realized_period_mean),
        "unfilled_entry_rate": _or_zero(unfilled_entry_mean),
        "forced_exit_rate": _or_zero(forced_exit_mean),
        "MinuteUnfilledEntryMean": _or_zero(unfilled_entry_mean),
        "MinuteForcedExitMean": _or_zero(forced_exit_mean),
    }
    return metrics
