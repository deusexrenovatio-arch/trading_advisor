...
def _pr_surface_with_task_contract(changed_files: list[str]) -> dict[str, object]:
    return {
        "mapping_path": "configs/change_surface_mapping.yaml",
        "changed_files": changed_files,
        "surfaces": ["contracts"],
        "primary_surface": "contracts",
        "docs_only": False,
        "surface_details": {"contracts": changed_files},
        "commands": {
            "loop": ["python scripts/validate_task_request_contract.py"],
            "pr": ["python scripts/run_loop_gate.py"],
            "nightly": ["python scripts/run_pr_gate.py"],
        },
    }
...
def _docs_surface_with_task_contract(changed_files: list[str]) -> dict[str, object]:
    payload = _docs_surface(changed_files)
    payload["commands"] = {
        "loop": ["python scripts/validate_task_request_contract.py"],
        "pr": ["python scripts/run_loop_gate.py"],
...
def test_run_loop_gate_scopes_task_request_contract_with_explicit_changed_files(monkeypatch) -> None:
    captured: dict[str, list[str]] = {}
    changed_files = ["contracts/foo.yaml"]
    ...
    monkeypatch.setattr(
        run_loop_gate,
        "compute_surface",
        lambda _changed_files, mapping_path: _pr_surface_with_task_contract(changed_files),
    )
...
def test_run_loop_gate_skips_task_contract_validation_for_docs_only_diff(monkeypatch) -> None:
    captured: dict[str, list[str]] = {}
    changed_files = ["docs/README.md"]
    ...
    monkeypatch.setattr(
        run_loop_gate,
        "compute_surface",
        lambda _changed_files, mapping_path: _docs_surface_with_task_contract(changed_files),
    )
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["run_loop_gate.py", "--changed-files", "docs/README.md", "--skip-session-check"],
    )

    assert run_loop_gate.main() == 0
    scoped = " ".join(captured["commands"])
    assert "validate_task_request_contract.py" not in scoped
    assert "validate_session_handoff.py" not in scoped
